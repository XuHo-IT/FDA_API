...
namespace FDAAPI.App.FeatG47_FrequencyAggregation
{
    public class ScheduledFrequencyAggregationHandler : IFeatureHandler<ScheduledFrequencyAggregationCommand, UnitResponse>
    {
        ...
        public ScheduledFrequencyAggregationHandler(
            IMediator mediator,
            ILogger<ScheduledFrequencyAggregationHandler> logger)
        {
            _mediator = mediator;
            _logger = logger;
        }
        ...
        public async Task<UnitResponse> ExecuteAsync(ScheduledFrequencyAggregationCommand request, CancellationToken cancellationToken)
        {
            ...
            _logger.LogInformation("Scheduling frequency aggregation. Mode: {Mode}, StartUtc: {Start}, EndUtc: {End}", request.Mode, startUtc, endUtc);

            await _mediator.Send(new FrequencyAggregationRequest(
                bucketType,
                startUtc,
                endUtc,
        ...
        {
            // Monday=1..Sunday=7; ensure previous completed week Mon->Sun
