#!/bin/bash
# Backup UAT Schema Script
# Usage: ./backup_uat_schema.sh
...
echo "🔄 Starting UAT schema backup..."
echo "📅 Date: $(date)"
...
if [ $? -eq 0 ] && [ -f "$BACKUP_FILE" ] && [ -s "$BACKUP_FILE" ]; then
    ...
    find "$BACKUP_DIR" -name "uat_schema_*.sql.gz" -type f -mtime +$RETENTION_DAYS -delete
